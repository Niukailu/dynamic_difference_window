...
def main():
    ...
    p.add_argument(
        "--continue-search",
        action="store_true",
        help="select new windows after the reference schedule ends",
    )
    p.add_argument(
    ...
    if (
        rounds < 1
        or (rounds > len(schedule) and not args.continue_search)
    ...
    try:
        with args.output.open("x") as out:
            ...
            for number in range(1, rounds + 1):
                reference = schedule[number - 1] if number <= len(schedule) else None
                cp.cuda.Device(devices[0]).use()
                start = time.perf_counter()
                target = (
                    Window(
                        int(reference["window_base"], 0),
                        tuple(reference["window_bits"]),
                    )
                    if reference
                    else None
                )
                if shards is None and min(
                    1 << len(left.bits), 1 << len(target.bits if target else left.bits)
                ) >= len(devices):
                    shards = cluster.split(prob)
                    prob = None
                selection_start = time.perf_counter()
                candidates = [target]
                if args.width is not None or reference is None:
                    if shards is None:
